...
class EventTextFormatter {
    ...
    private static function formatRecord_create($event, $params, $htmlOptions) {
        if (X2Model::getModelOfTypeWithId($event->associationType,
            ...
            if ($event->associationType === 'Actions') {
                return static::formatRecordCreateActions($event, $params,
                                $htmlOptions);
            } else {
                return static::formatRecordCreateGeneric($event, $params,
                                $htmlOptions);
            }
        } else {
            return static::formatRecordCreateDeleted($event, $params,
                            $htmlOptions);
        }
    }
    ...
    private static function formatRecordCreateActions($event, $params, $htmlOptions) {
        ...
        if ((strcasecmp($action->associationType, 'contacts') === 0 ||
            ...
            return static::formatContactAction($event, $action, $params,
                            $htmlOptions);
        } else {
            return static::formatGenericAction($event, $action, $params,
                            $htmlOptions);
        }
    }

    private static function formatContactAction($event, $action, $params, $htmlOptions) {
        $requireAbsoluteUrl = (array_key_exists('requireAbsoluteUrl', $params)) ? $params['requireAbsoluteUrl']
                    : false;
        ...
        if (method_exists('EventTextFormatter', $actionFmtFn)) {
            ...
            return static::formatContactActionGeneric($event, $action,
                            $requireAbsoluteUrl, $htmlOptions);
        }
    }
    ...
    private static function formatCallAction($action, $requireAbsoluteUrl = false) {
        ...
        return Yii::t('app',
                    ...
                    '{duration}' => empty($action->dueDate) || empty($action->completeDate)
                                ? Yii::t('app', 'duration unknown') : Formatter::formatTimeInterval($action->dueDate,
                                    $action->completeDate, '{hoursMinutes}'),
                    '{text}' => $action->actionDescription,
                    '{modelLink}' => X2Model::getModelLink($action->associationId,
    ...
    private static function formatTimeAction($action, $requireAbsoluteUrl = false) {
        ...
        return Yii::t('app',
                        ...
                        array(
                    '{authorText}' => $authorText,
                    '{time}' => Formatter::formatTimeInterval($action->dueDate,
                            $action->dueDate + $action->timeSpent,
                            '{hoursMinutes}'),
                    '{text}' => $action->actionDescription,
                    '{modelLink}' => X2Model::getModelLink($action->associationId,
                            ucfirst($action->associationType))
    ...
    private static function formatContactActionGeneric($event, $action, $requireAbsoluteUrl
    = false, $htmlOptions = array()) {
        $authorText = User::getUserLinks($action->assignedTo);
        if ($authorText === Yii::t('app', 'Anyone')) {
            $authorText = Yii::t('app', 'Someone');
        }
        if ((!empty($authorText) && $authorText !== (Yii::t('app', 'Anyone'))) && $authorText
                != Yii::t('app', 'Someone')) {
            return Yii::t('app',
    ...
    private static function formatGenericAction($event, $action, $params, $htmlOptions) {
        ...
        if (!empty($authorText) && $authorText !== (Yii::t('app', 'Anyone') . " ")) {
            $modelLink = X2Model::getModelLink(
                            $event->associationId, $event->associationType,
                            $requireAbsoluteUrl);
            ...
            return $authorText . Yii::t('app', $translateText,
                            array(
                        '{modelName}' => Events::parseModelName($event->associationType),
                        '{modelLink}' => $modelLink,
                        '{assignee}' => isset($action) ? User::getUserLinks($action->assignedTo)
                                    : null,
            ));
        } else {
            return Yii::t('app',
                            "A new {modelName}, {modelLink}, has been created.",
                            array('{modelName}' => Events::parseModelName($event->associationType),
                        '{modelLink}' => X2Model::getModelLink($event->associationId,
                                $event->associationType, $requireAbsoluteUrl)));
        }
    }
    ...
    private static function formatWeblead_create($event, $params, $htmlOptions) {
        if (X2Model::getModelOfTypeWithId($event->associationType,
                        $event->associationId)) {
            return Yii::t('app', "A new web lead has come in: {modelLink}",
                            array(
                        '{modelLink}' => X2Model::getModelLink($event->associationId,
    ...
    private static function formatRecord_deleted($event, $params, $htmlOptions) {
        ...
        if ((!empty($authorText) && $authorText !== (Yii::t('app', 'Anyone') . " "))) {
            ...
            return Yii::t('app', "The {modelType}, {text}, was deleted.",
                            array(
                        '{modelType}' => Events::parseModelName($event->associationType),
                        '{text}' => $event->text
            ));
    ...
    private static function formatFeed($event, $params, $htmlOptions) {
        ...
        return $authorText . $modifier . $recipient . ": " . ($truncated ? strip_tags(Formatter::convertLineBreaks(x2base::convertUrls($event->text),
                                        true, true), '<a></a>') : $event->text);
    }
    ...
    private static function formatEmailSentQuote($event, $params, $htmlOptions) {
        $authorText = static::getAuthorText($event, $htmlOptions);
        $model = X2Model::getModelOfTypeWithId($event->associationType,
                        $event->associationId);
        if ($model) {
            return $authorText . Yii::t('app',
    ...
    private static function formatEmail_opened($event, $params, $htmlOptions) {
        ...
        if (X2Model::getModelOfTypeWithId($event->associationType,
                        $event->associationId)) {
            return X2Model::getModelLink($event->associationId,
    ...
    private static function formatCalendar_event($event, $params, $htmlOptions) {
        ...
        if (!Yii::app()->params->isMobileApp ||
            ...
            $calendarText = CHtml::link(
                            ...
                            Yii::app()->absoluteBaseUrl . '/index.php' . '/calendar/calendar/index',
                            $htmlOptions);
        } else {
            $calendarText = Yii::t('calendar', 'Calendar');
        }
        if (isset($action)) {
            return Yii::t('app', "{calendarText} event: {actionDescription}",
                            array(
                        '{calendarText}' => $calendarText,
                        '{actionDescription}' => CHtml::encode($action->actionDescription)
            ));
        } else {
            return Yii::t('app', "{calendarText} event: event not found.",
                            array(
                        '{calendarText}' => $calendarText
            ));
        }
    ...
    private static function formatAction_reminder($event, $params, $htmlOptions) {
        ...
        if (isset($action)) {
            return Yii::t('app',
                            ...
                            array(
                        '{transModelLink}' => X2Model::getModelLink($event->associationId,
                                $event->associationType),
                        '{action}' => strtolower(Modules::displayName(false,
                                        'Actions')),
            ));
        } else {
            return Yii::t('app',
    ...
    private static function formatDoc_update($event, $params, $htmlOptions) {
        ...
        if ($doc) {
            return $authorText . Yii::t('app', 'updated a document, {docLink}',
                            array(
                        '{docLink}' => X2Model::getModelLink($event->associationId,
                                $event->associationType)
            ));
        } else {
            return $authorText . Yii::t('app',
    ...
    private static function formatContactEmail($action, $authorText) {
        ...
        $text = $authorText . Yii::t('app', "received an {email} from ",
                        array(
                    '{email}' => $email,
        ));
        ...
        foreach (array('Contacts', 'Services') as $modelType) {
            if (array_key_exists($modelType, $modelLinks) && !empty($modelLinks[$modelType])) {
                ...
                $modelDescription = lcfirst(Modules::displayName($multiple,
                                $modelType));
                if ($multiple) {
                    $modelDescription .= ',';
                }
                $text .= Yii::t('app', 'the {transModelName} {modelLinks}',
                                array(
                            '{transModelName}' => $modelDescription,
                            '{modelLinks}' => implode(', ',
                                    $modelLinks[$modelType]),
                ));

                if ($modelType === 'Contacts' && array_key_exists('Services',
                                $modelLinks) &&
    ...
    private static function formatVoip_call($event, $params, $htmlOptions) {
        if (X2Model::getModelOfTypeWithId($event->associationType,
                        $event->associationId)) {
            return Yii::t('app', "{modelLink} called.",
                            array(
                        '{modelLink}' => X2Model::getModelLink($event->associationId,
                                $event->associationType)
            ));
    ...
    private static function formatTopic_reply($event, $params, $htmlOptions) {
        ...
        if (isset($reply)) {
            ...
            return Yii::t('topics', '{poster}posted a new reply to {topic}.',
                            array(
                        '{poster}' => $authorText,
                        '{topic}' => $topicLink,
            ));
    ...
    public static function getAuthorText($event, $htmlOptions = array()) {
        ...
        if (Yii::app()->user->getName() == $event->user) {
            $authorText = CHtml::link(
                            Yii::t('app', 'You'), $event->profile->url,
                            $htmlOptions);
        } else {
